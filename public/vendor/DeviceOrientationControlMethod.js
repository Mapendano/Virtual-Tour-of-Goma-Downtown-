...
window.DeviceOrientationControlMethod = function() {
  this._dynamics = {
    yaw: new window.Marzipano.Dynamics(),
    pitch: new window.Marzipano.Dynamics()
  };
...
window.DeviceOrientationControlMethod.prototype.getPitch = function(cb) {
  this._getPitchCallbacks.push(cb);
};


window.DeviceOrientationControlMethod.prototype._handleData = function(data) {
  var previous = this._previous,
      current = this._current,
      tmp = this._tmp;

  tmp.yaw = window.Marzipano.util.degToRad(data.alpha);
  tmp.pitch = window.Marzipano.util.degToRad(data.beta);
  tmp.roll = window.Marzipano.util.degToRad(data.gamma);
